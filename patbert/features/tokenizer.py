...
class BaseTokenizer():
    ...
    def batch_encode(self, seqs):
        seqs['idx'] = []
        for i in tqdm(range(len(seqs['concepts']))):
            seqs['idx'].append(self.encode_seq(seqs['concepts'][i]))
        del seqs['concepts']
        return seqs
...
class EHRTokenizer():
    ...
    def __init__(self, vocabulary=None):
        ...
        if isinstance(vocabulary, type(None)):
            ...
            for i in range(1900, 2022):
                self.vocabulary[f'[BIRTHYEAR_{i}]'] = len(self.vocabulary)
            for i in range(1, 13):
                self.vocabulary[f'[BIRTHMONTH_{i}]'] = len(self.vocabulary)
        else:
            self.vocabulary = vocabulary
    ...
    def __call__(self, seq):        
        ...
        self.seq = seq
        first_visit = 1
        abs_pos_0 = seq['abspos'][0]
        last_abs_pos = 0

        # skip pid, birthdate and sex
        for i in range(len(seq['concepts'])):
            if seq['segment'][i] > first_visit:
                ...
                self.append_previous_token('abspos')
                last_abs_pos = seq['abspos'][i]
                self.enc_seq['values'].append(1)
                first_visit = seq['segment'][i]
                if len(seq['abspos']) > (i + 1):
                    abs_pos_0 = seq['abspos'][i + 1]  # abs pos of next visit
            for key in ['segment', 'concepts', 'ages', 'abspos', 'values']:
                self.append_token_from_original(key, i)
            self.enc_seq['idx'].append(self.encode(seq['concepts'][i]))
            if i == (len(seq['concepts']) - 1):
                self.enc_seq['los'].append(seq['abspos'][i] - last_abs_pos)
    ...
    def append_code_idx(self, code):
        ...
        self.enc_seq['concepts'].append(code)
    ...
    def insert_code_idx(self, code):
        ...
        self.enc_seq['concepts'].insert(0, code)
    ...
    def truncate(self):
        ...
        if not isinstance(self.max_len, type(None)):
            ...
            if len(self.enc_seq['idx']) > max_len:
                if self.enc_seq['concepts'][-max_len] == '[SEP]':
                    # we don't want to start a sequence with SEP
                    max_len = max_len - 1
                for key in ['concepts', 'idx', 'values',
